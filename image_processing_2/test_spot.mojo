# Test Spot Benchmark — V2 stages (optimized Grayscale, GaussianBlur, Sharpen)
#
# Structure:
...
#   Phase 3: Optimal config   — G=2 B=7 S=10  (tuned for V2 stage costs)
#
# V2 stage costs: Gray=0.617ms  Blur=2.219ms  Sharp=2.939ms  per image
...
# ============================================================================
# Phase 3: Optimal config — G=2 B=7 S=10  (21 total threads)
# ============================================================================
fn run_optimal() raises -> Tuple[Int, Float64]:
    ...
    var sink = ImageSink()
    var count_ptr = sink.count_ptr
    var pipeline = Pipeline((seq(source), parallel(gray, 2), parallel(blur, 7), parallel(sharp, 10), seq(sink)))
    pipeline.setPinning(True)
    var t0 = perf_counter_ns()
    pipeline.run()
    var ms = elapsed_ms(t0)
...
def main():
    print("=" * 70)
    print("  Test Spot Benchmark (V2 stages)")
    ...
    print_row("OPT  G2  B7  S10", 21, n_opt, t_opt, tput_opt, tput_source)
    ...
    print("  Optimal G2B7S10:" + String(tput_opt) + " img/s  (" + String(tput_opt / tput_source * 100.0) + "%)")
    ...
    print("optimal_g2b7s10," + String(n_opt) + "," + String(t_opt) + "," + String(tput_opt) + "," + String(tput_opt / tput_source * 100.0))
    print("CSV_END")
