"""
Plot script for source_rate_benchmark results (time-based source).
...
def plot_throughput(df):
    ...
    for size in sizes:
        ...
        for cfg, lbl, col in CONFIG_DISPLAY:
            ...
            colors.append(col)
            nimgs.append(int(row.iloc[0]['num_images']))
        ...
        for bar, tput, nimg in zip(bars, tputs, nimgs):
            eff = tput / source_tput * 100.0
            ax.text(bar.get_x() + bar.get_width() / 2., bar.get_height() + source_tput * 0.01,
                    f'{eff:.0f}%\n({nimg} imgs)', ha='center', va='bottom', fontsize=7, fontweight='bold')
        ax.set_xticks(list(x))
        ax.set_xticklabels(labels, fontsize=8)
        ax.set_title(f'Throughput vs Source Ceiling  ({size}, 60s duration)\n'
...
def plot_efficiency(df):
    ...
    for size in sizes:
        ...
        ax.set_title(f'Efficiency vs Threads Used  ({size}, 60s duration)',
                     fontsize=12, fontweight='bold')
...
def plot_speedup(df):
    ...
    for size in sizes:
        ...
        ax1.set_title(f'Speedup vs Sequential  ({size}, 60s duration)\n'
