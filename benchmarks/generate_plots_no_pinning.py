...
def plot_scalability(df):
    ...
    for size in sizes:
        ...
        ax.set_title(f'Pipeline Scalability (No Pinning) — Payload {size}B', fontsize=14)
        ax.legend(fontsize=10)
        ax.grid(True, alpha=0.3)
        ax.set_xticks(range(2, 13))
        plt.tight_layout()
        fig.savefig(f'plots/scalability_{size}B_no_pinning.png', dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  Generated plots/scalability_{size}B_no_pinning.png")
...
def plot_efficiency_degradation(df):
    ...
    ax.set_title(f'Efficiency Degradation vs. Computation Time (No Pinning)\n({ref_queue}, Payload {ref_size}B)',
                 fontsize=13)
    ax.legend(fontsize=10)
    ax.grid(True, alpha=0.3)
    ...
    ax.set_xticks(range(2, 13))
    plt.tight_layout()
    fig.savefig('plots/efficiency_degradation_no_pinning.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
    print("  Generated plots/efficiency_degradation_no_pinning.png")
...
def plot_overhead_histograms(df):
    ...
    for n in n_values:
        ...
        ax.set_title(f'Queue Overhead (Zero Computation, No Pinning) — N={n}', fontsize=14)
        ax.set_xticks(x)
        ...
        ax.set_yscale('log')
        plt.tight_layout()
        fig.savefig(f'plots/overhead_N{n}_no_pinning.png', dpi=150, bbox_inches='tight')
        plt.close(fig)
        print(f"  Generated plots/overhead_N{n}_no_pinning.png")
...
if __name__ == "__main__":
    ...
    scal_file = 'results/scalability_results_no_pinning.txt'
    ...
    bench_file = 'results/benchmark_results_no_pinning.txt'
    ...
    print("\nDone generating no pinning extra plots!")
