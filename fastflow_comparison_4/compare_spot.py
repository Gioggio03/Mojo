"""
Comparison plot: Mojo (MoStream V4) vs FastFlow V4 — usando i risultati dello spot sweep.

Legge spot_sweep_512_O3.txt di FastFlow (ogni config = processo separato)
e benchmark_full_512_O3.txt di Mojo V4 per confronto.
...
RESULTS_DIR   = 'results'
MOJO_RESULTS  = os.path.join('..', 'image_processing_4', 'results', 'benchmark_full_512_O3.txt')
...
_CONFIG_MAP = {
    ...
    'g6b6s6':    'uniform_p6',
    'g7b7s7':    'uniform_p7',
    'g2b7s10':   'optimal_g2b7s10',  # FF4 optimal config
}

# Note: Mojo V4 optimal is G3 B8 S8 (optimal_g3b8s8);
#       FF4 spot sweep tested G2 B7 S10 (optimal_g2b7s10).
...
def plot_throughput(df):
    ...
    ax.set_title('Throughput: Mojo V4 (MoStream) vs FastFlow V4 — Spot Sweep\n'
    ...
    legend = [mpatches.Patch(color=MOJO_COLOR, label='Mojo V4 (MoStream)'),
              mpatches.Patch(color=FF_COLOR,   label='FastFlow V4')]
...
def plot_speedup(df):
    ...
    ax.set_title('FastFlow V4 / Mojo V4 Throughput Ratio — Spot Sweep\n'
...
def plot_latency(df):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([lbl for _, lbl in CONFIGS], fontsize=7)
    ax.set_title('Latency per Image: Mojo V4 vs FastFlow V4 — Spot Sweep\n(lower is better)',
                 fontsize=13, fontweight='bold')
...
def print_summary(df):
    print('\n' + '=' * 72)
    print('  SUMMARY: Mojo V4 (MoStream) vs FastFlow V4 — Spot Sweep')
    print('=' * 72)
