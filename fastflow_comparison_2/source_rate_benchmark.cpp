...
int main() {
    ...
    int images_uniform[6];
    double tput_uniform[6];
    double time_uniform[6];
    for (int i = 0; i < 6; i++) {
        auto& c = uniform_configs[i];
        std::printf("  [Running] %s...\n", c.label); std::fflush(stdout);
        auto res = run_pipeline(c.gray_p, c.blur_p, c.sharp_p);
        images_uniform[i] = res.images;
        tput_uniform[i] = throughput(res.images, res.time_ms);
        time_uniform[i] = res.time_ms;
        print_row(c.label, c.total_threads, res.images, res.time_ms, tput_uniform[i], tput_source);
        std::fflush(stdout);
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        std::printf("  [Running] %s...\n", c.label); std::fflush(stdout);
        auto res = run_pipeline(c.gray_p, c.blur_p, c.sharp_p);
        images_smart[i] = res.images;
        tput_smart[i] = throughput(res.images, res.time_ms);
        time_smart[i] = res.time_ms;
        print_row(c.label, c.total_threads, res.images, res.time_ms, tput_smart[i], tput_source);
        std::fflush(stdout);
    }
    ...
    std::printf("  Best uniform P=7: %.2f img/s  (%.1f%%)  [%d threads, %d images]\n",
                tput_uniform[5], tput_uniform[5] / tput_source * 100.0, 23, images_uniform[5]);
    std::printf("  Best smart:       %.2f img/s  (%.1f%%)  [%d threads, %d images]\n",
                tput_smart[3], tput_smart[3] / tput_source * 100.0, 24, images_smart[3]);
    std::printf("======================================================================\n");

    // CSV output (same format as Mojo for comparison)
    std::printf("\nCSV_START\n");
    std::printf("config,gray_p,blur_p,sharp_p,total_threads,num_images,time_ms,throughput,efficiency_pct\n");
    std::printf("source_baseline,0,0,0,3,%d,%.6f,%.6f,100.0\n", res0.images, res0.time_ms, tput_source);
    std::printf("seq,1,1,1,5,%d,%.6f,%.6f,%.6f\n",
    ...
    for (int i = 0; i < 6; i++) {
        ...
        std::printf("%s,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f\n",
                    ...
                    c.total_threads, images_uniform[i], time_uniform[i], tput_uniform[i],
    ...
    for (int i = 0; i < 4; i++) {
        ...
        std::printf("%s,%d,%d,%d,%d,%d,%.6f,%.6f,%.6f\n",
                    ...
                    c.total_threads, images_smart[i], time_smart[i], tput_smart[i],
