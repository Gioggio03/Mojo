"""
Comparison plot: Mojo (MoStream) vs FastFlow — usando i risultati dello spot sweep.

Differenza rispetto a compare_results.py: legge spot_sweep_512_O3.txt di FastFlow
(ogni config = processo separato, senza accumulo di thread tra run).

Genera gli stessi 3 grafici in results/plots_spot/.
...
RESULTS_DIR   = 'results'
MOJO_RESULTS  = os.path.join('..', 'image_processing_2', 'results', 'benchmark_full_512_O3.txt')
FF_SPOT_FILE  = os.path.join(RESULTS_DIR, 'spot_sweep_512_O3.txt')
PLOTS_DIR     = _args.plots_dir or os.path.join(RESULTS_DIR, 'plots_spot')
os.makedirs(PLOTS_DIR, exist_ok=True)

# Map gXbYsZ config names → canonical keys used in CONFIGS list
_CONFIG_MAP = {
    'g1b1s1':    'seq',
    'g2b2s2':    'uniform_p2',
    'g3b3s3':    'uniform_p3',
...
def parse_spot_sweep(filepath):
    """
    Parse spot_sweep_512_O3.txt: contains multiple CSV_START/END blocks,
    one per config run. Each block has a source_baseline row and one config row.
    Returns list of dicts with keys: config, throughput, num_images, source_tput.
    """
    rows = {}          # canonical_name -> dict
    src_tputs = []     # all source baseline measurements
...
def parse_mojo_benchmark(filepath):
    """
    Parse Mojo benchmark_full_512_O3.txt (5-column simplified CSV).
    Returns dict: config -> {throughput, num_images, ...} + source_tput.
    """
    rows = {}
    source_tput = 0.0
    in_csv = False
...
def plot_throughput(df):
    ...
    ax.set_title('Throughput: Mojo (MoStream) vs FastFlow — Spot Sweep\n'
    ...
    legend = [mpatches.Patch(color=MOJO_COLOR, label='Mojo (MoStream)'),
...
def plot_speedup(df):
    ...
    ax.set_title('FastFlow / Mojo Throughput Ratio — Spot Sweep\n'
...
def plot_latency(df):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([lbl for _, lbl in CONFIGS], fontsize=7)
    ax.set_title('Latency per Image: Mojo vs FastFlow — Spot Sweep\n(lower is better)',
                 fontsize=13, fontweight='bold')
...
def print_summary(df):
    print('\n' + '=' * 72)
    print('  SUMMARY: Mojo (MoStream) vs FastFlow — Spot Sweep (V2 stages)')
    print('=' * 72)
